...
async function markDirtyTiles(tilesDir) {
  ...
  for (const tile of deepTiles) {
    ...
    if (now - lastLog > 15000) {
      console.log(`Processing dirty tile ${i} of ${deepTiles.length} (${(i / deepTiles.length * 100).toFixed()} %)`);
      lastLog = now;
    }
    ...
    if (!tileOverlapsLimits(prerenderPolygon, til) || til.zoom < prerenderConfig.minZoom || til.zoom > prerenderConfig.maxZoom) {
      for (const scale of limitScales) {
        try {
          await unlink(path.resolve(tilesDir, `${tile}${scale === 1 ? '' : `@${scale}x`}.${extension}`));
        } catch (_) {
          // ignore
        }
      }
    } else if (await exists(path.resolve(tilesDir, `${tile}.${extension}`))) { // TODO this seems to be very slow if rendering is in progress (CPU)
