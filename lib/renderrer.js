...
async function renderSingleScale(p, zoom, x, y, scale, prerender) {
  ...
  const ccnt = cnt++;
  const tmpName1 = `${ps}_${ccnt}_tmp1.png`;
  const tmpName2 = `${ps}_${ccnt}_tmp2.png`;
  ...
  try {
    ...
    await map.renderFileAsync(tmpName1, { format: 'png', buffer_size: 256, scale });
    await execFileAsync(pngquant, ['-o', tmpName2, tmpName1]);
  } finally {
    getPool().release(map);
  }

  await unlink(tmpName1);
  await rename(tmpName2, `${ps}.png`).catch((err) => {
