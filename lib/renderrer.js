...
async function renderTile(zoom, x, y, reqScale) {
  ...
  return reqScale ? `${p}${reqScale === 1 ? '' : `@${reqScale}x`}.${extension}` : undefined;
}
...
async function shouldRender(p, tile) {
  let s;
  try {
    s = await stat(`${p}${tile.reqScale === 1 ? '' : `@${tile.reqScale}x`}.${extension}`);
  } catch (err) {
 ...
 */
async function exportMap(destFile, xml, zoom, bbox0, scale = 1, width, cancelHolder, format) {
  ...
  try {
    ...
    map.zoomToBox(bbox);
    await map.renderFileAsync(destFile, { format, buffer_size: 256, scale_denominator: zoomDenoms[zoom], scale });
  } finally {
