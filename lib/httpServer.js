...
router.post('/export', koaBody, async (ctx) => {
  ...
  if (geojson) {
    shapefilesDir = await fs.mkdir(
      path.join(os.tmpdir(), `shapefiles-${token}`),
    );
...
router.head('/export', koaBody, async (ctx) => {
...
router.get('/export', koaBody, async (ctx) => {
  const job = jobMap.get(ctx.query.token);
...
router.delete('/export', koaBody, async (ctx) => {
  const job = jobMap.get(ctx.query.token);
