...
// scale - size of the labels - independent of the width or bbox
router.get('/pdf', mapExporter); // for compatibility

router.get('/export', mapExporter);

/**
 * @param {RouterContext} ctx
 */
async function mapExporter(ctx) {
  const q = ctx.query;
  ...
  if (
    ...
    zoom > 20 ||
    bbox.length !== 4 ||
    bbox.some(c => Number.isNaN(c))
  ) {
    ctx.status = 400;
    return;
  }

  const filename = `export-${tmpIndex++}.${q.format || 'pdf'}`;
