...
const schema = {
  ...
  properties: {
    ...
    features: {},
    scale: { type: 'mumber', minimum: 0.1, maximum: 10 },
...
router.post('/export', koaBody, async (ctx) => {
  ...
  const filename = `export-${tmpIndex++}.${format}`;
  ...
  try {
    ...
    if (geojson) {
      dir = await fs.mkdtemp(path.join(os.tmpdir(), 'shapefiles-'));
      ...
      await Promise.all(
        [
          ...
          const prefix = path.join(dir, shpType.toLowerCase());

          await Promise.all(
            ['shp', 'shx', 'dbf'].map((ext) =>
              fs.writeFile(`${prefix}.${ext}`, files[ext].buffer),
            ),
          );

          shapefiles[type] = `${prefix}.shp`;
        }),
      );
    }
    ...
    await Promise.all([
      ...
      dir && fs.rmdir(dir, { recursive: true }),
    ]);
  }
});
