...
async function fillDirtyTilesRegister() {
  ...
  for (const { zoom, x, y } of tileRangeGenerator(prerenderPolygon, minZoom, maxZoom)) {
    /** @type {number} */
    let mtimeMs;
    try {
      // find oldest
      const proms = limitScales.map((scale) => stat(path.join(tilesDir, `${zoom}/${x}/${y}${scale === 1 ? '' : `@${scale}x`}.${extension}`)));
      mtimeMs = Math.min(...(await Promise.all(proms)).map(stat => stat.mtimeMs));
    } catch (e) {
