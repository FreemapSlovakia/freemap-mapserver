...
async function processExpireFiles(tilesDir) {
  ...
  async function processTile(til) {
  ...
  await Promise.all(
    outzoomExpiredTiles.map((til) =>
      processTile(til).catch((err) => {
        console.error('Error expiring tile: ', til, err);
      }),
    ),
  );

  await Promise.all(expireFiles.map((ff) => unlink(ff)));
